...
export default function TaskDetail({ taskId, projectId }: TaskDetailProps) {
  const { loading, request } = useApi<Comment | Task>();
  const { user } = useUserStore();
  const { loading: isFetchTaskLoading, request: isFetchTaskRequest } =
    useApi<Task>();
  ...
  const addComment = async () => {
    ...
    await request(
      ...
      },
      {
        onSuccess: (data) => {
          if (data?.data) {
            setComments([...comments, data.data as Comment]);
          }
          setNewComment('');
        },
      }
    ...
    await isFetchTaskRequest(
      ...
      },
      {
        onSuccess: (data) => {
          ...
          setProject(
            result?.project?.name ?? result?.project ?? 'First PROJECT'
          );
          setAssigned(result?.assignedTo?.id || null);
  ...
  return (
    <div className="min-h-screen bg-zinc-50/40 py-8 px-4 md:px-8 relative">
      <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border shadow-sm">
                <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                  {task?.assignedTo.name?.slice(0, 2).toLocaleUpperCase() ||
                    'T'}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {!isFetchTaskLoading ? (
            ...
            <Skeleton className="w-full lg:col-span-2 min-h-[950px]" />
          )}
        ...
        <div className="flex justify-end pt-6">
          <Button onClick={handleSave} size="lg" className="px-10 py-6 text-lg">
            Save Changes
          </Button>
        </div>
      </div>
